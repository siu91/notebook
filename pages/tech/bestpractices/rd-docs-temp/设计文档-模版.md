...
3. 文档组成：文档由软件设计相关的架构、流程图、数据模型、算法步骤、接口说明等组成。
...
4. 兼容性
   ...
   > ⚠️ **C 端系统的开发，有时候比较麻烦的是低版本app的兼容，尽可能早期设计的时候，就考虑可能的扩展，如果实在没法兼容，那就只能app强制更新，当然这种用户体验就非常不好了。**
...
```mermaid
    ...
    Container_Boundary(c1, "客户展示层") {
        Container(spa, "站内（弹屏、站内信）")
        Container(mobile_app, "站外（邮件、短信）")
    }

    Container_Boundary(c2, "核心业务模块层") {
        Container(web_app, "基础设置")
        Container(task_manager, "任务管理")
        Container(scene_setting, "场景设置")
        Container(message_management, "消息管理")
    }

    Container_Boundary(c3, "数据") {
        ContainerDb(data_collection, "数据收集")
        ContainerDb(user_tags, "用户标签")
        ContainerDb(user_group, "用户分群")
        ContainerDb(data_statistics, "数据统计")
        ContainerDb(recommendation_algorithm, "推荐算法")
    }
...
```mermaid
    ...
    Deployment_Node(plc, "xxx 服务端部署架构", "xxx 数据中心"){
